...
app.prepare().then(() => {
  ...
  server.use(
    createShopifyAuth({
      ...
      scopes: ['read_products','read_script_tags','write_script_tags','read_orders','write_discounts'],
      async afterAuth(ctx) {
        ...
        dd === 0 ? ctx.redirect('/') : await getSubscriptionUrl(ctx, accessToken, shop);
      }
    })
  );
  ...
  router.get('/charge/:object', async (ctx) => {
     const results = await fetch("https://" + ctx.cookies.get('shopOrigin') + "/admin/api/2020-01/recurring_application_charges/"+ctx.params.object+".json" , {
        headers: {
  ...
  router.post('/webhooks/subscription/update', webhook, (ctx) => {
    ...
    updateSubscription(ctx.request.body.app_subscription.admin_graphql_api_shop_id, ctx.request.body.app_subscription.status,ctx.request.body.app_subscription.updated_at);//ctx.request.body.app_subscription.status
  });
